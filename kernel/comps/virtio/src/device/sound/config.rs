...
impl ConfigManager<VirtioSoundConfig> {
    pub(super) fn read_config(&self, ctls_negotiated: bool) -> VirtioSoundConfig {
        ...
        sound_config.jacks = self
            .read_once::<u32>(offset_of!(VirtioSoundConfig, jacks) as usize)
            .unwrap_or(0);
        sound_config.streams = self
            .read_once::<u32>(offset_of!(VirtioSoundConfig, streams) as usize)
            .unwrap_or(0);
        sound_config.chmaps = self
            .read_once::<u32>(offset_of!(VirtioSoundConfig, chmaps) as usize)
            .unwrap_or(0);
        if ctls_negotiated {
            sound_config.controls = self
                .read_once::<u32>(offset_of!(VirtioSoundConfig, controls) as usize)
                .unwrap_or(0);
        } else {
